  ...
  useEffect(() => {
    ...
    if (isValidConversation) {
      ...
      const handleUserUnblocked = (data: {
        ...
        // Lấy tên người bị chặn
        const unblockedUserName =
          userCache[data.unblockedUserId]?.fullname || "Người dùng";
        const currentUserId = localStorage.getItem("userId");
        const isCurrentUser =
          data.fromCurrentUser || data.unblockedUserId === currentUserId;

        // Tạo tin nhắn hệ thống
        const systemMessage = {
          type: "system",
          content: isCurrentUser
            ? `Bạn đã được bỏ chặn khỏi nhóm`
            : `${unblockedUserName} đã được bỏ chặn khỏi nhóm`,
          senderId: "system",
          createdAt: new Date().toISOString(),
        };

        // Cập nhật tin nhắn trong context để hiển thị ngay lập tức
      ...
      const handleUserRemovedFromGroup = (data: {
        ...
        const removedUser = userCache[data.kickedUser.userId] || {
          fullname: data.kickedUser.fullname,
        };
        const removedUserName = removedUser?.fullname || "Một thành viên";

        // Lấy thông tin về người xóa
        const remover = userCache[data.kickedByUser.userId] || {
          fullname: data.kickedByUser.fullname,
        };
