...
const reducer = (state: State, action: Action): State => {
  switch (action.type) {
    ...
    case "TOGGLE_ADD_CO_OWNER_MODAL":
      return {
        ...state,
        isAddCoOwnerModalVisible: !state.isAddCoOwnerModalVisible,
      };
    ...
    case "TOGGLE_TRANSFER_OWNER_MODAL":
      return {
        ...state,
        isTransferOwnerModalVisible: !state.isTransferOwnerModalVisible,
      };
    ...
    case "UPDATE_USER_CACHE":
      return {
        ...state,
        localUserCache: { ...state.localUserCache, ...action.payload },
      };
    case "FORCE_RENDER":
...
const useSocketEvents = (
  ...
  // const currentUserId = localStorage.getItem("userId") || "";
  const handleGroupCoOwnerAdded = useCallback(
    (data: {
      conversationId: string;
      ...
      if (
        JSON.stringify(existingCoOwnerIds) ===
        JSON.stringify(data.newCoOwnerIds)
      )
        return;
  ...
  const handleGroupCoOwnerRemoved = useCallback(
    (data: { conversationId: string; removedCoOwner: string }) => {
      if (data.conversationId !== conversationId) return;
      // Gọi lại API để lấy conversation mới nhất
      getConversationDetail(conversationId).then((updatedConversation) => {
        if (updatedConversation) {
  ...
  const handleGroupOwnerChanged = useCallback(
    (data: { conversationId: string; newOwner: string }) => {
      if (data.conversationId !== conversationId) return;
      getConversationDetail(conversationId).then((updatedConversation) => {
        if (updatedConversation) {
          dispatch({
...
// Định nghĩa interface cho dữ liệu tin nhắn mới
interface NewMessageData {
  conversationId: string;
  message?: any; // Sử dụng any cho message vì cấu trúc có thể phức tạp
...
const GroupManagement: React.FC<GroupManagementProps> = ({
  ...
  // Cập nhật hàm handleNewMessage với kiểu dữ liệu
      ...
      onOk: async () => {
        ...
        try {
          ...
          notification.open({
            ...
            description: t.disbanding_group || "Đang giải tán nhóm...",
            duration: 0,
          });
          ...
          if (result) {
            notification.success({
              key,
              message: t.success || "Thành công",
              description:
                t.group_disbanded_success || "Đã giải tán nhóm thành công.",
              duration: 2,
            });
            ...
            notification.error({
              ...
              message: t.error || "Lỗi",
              description:
                error || t.cannot_disband_group || "Không thể giải tán nhóm",
              duration: 2,
            });
          }
        } catch (err) {
  ...
  const regularMembers =
    conversation.groupMembers?.filter(
      (memberId) => memberId !== owner && !coOwners.includes(memberId)
    ) || [];
        ...
        dispatch({
          ...
          payload: {
            ...
            rules: {
              ...
              ownerId: conversation.rules?.ownerId || "",
              coOwnerIds:
                conversation.rules?.coOwnerIds?.filter(
                  (id) => id !== coOwnerId
                ) || [],
            },
          },
    ...
    try {
      ...
      if (updatedConversation) {
        ...
        notification.success({
          ...
          message: "Thành công",
          description:
            "Đã chuyển quyền trưởng nhóm thành công. Bạn có thể rời nhóm an toàn bây giờ.",
          duration: 5,
        });

        setTimeout(() => {
          Modal.info({
            title: t.transfer_success_title || "Chuyển quyền thành công",
            content:
              t.transfer_success_message ||
  ...
  const handleMemberBlockStatusChange = useCallback(
  ...
  // Thêm useEffect để xử lý thay đổi ngôn ngữ
  useEffect(() => {
    // Cập nhật UI khi ngôn ngữ thay đổi
    // Chỉ cần dispatch FORCE_RENDER để kích hoạt render lại
    dispatch({ type: "FORCE_RENDER" });
  }, [language, dispatch]);
  ...
  const renderOwnerCoOwnerView = () => {
    ...
    return (
      <div
        className="group-management flex flex-col h-full bg-white"
        key={`group-management-${renderKey}`}>
        {/* Header */}
        <div className="flex items-center p-4 border-b border-gray-200 relative">
          ...
          <div className="flex items-center justify-center w-full">
            <h2 className="text-lg font-semibold text-center">
              {t.owner_deputy || "Trưởng & phó nhóm"}
            </h2>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          {userRole === UserRole.OWNER && (
            <Button
              ...
              {t.transfer_owner || "Chuyển quyền trưởng nhóm"}
            </Button>
          )}
        </div>
      </div>
  ...
  const renderAddCoOwnerModal = () => {
    return (
      <Modal
        title={t.adjust_deputy || "Điều chỉnh phó nhóm"}
        ...
        <div className="py-2">
          <Input
            placeholder={t.search_member || "Tìm kiếm thành viên"}
          ...
          {filteredMembers.length === 0 && (
            <div className="text-center py-4 text-gray-500">
              {t.no_member_found || "Không tìm thấy thành viên"}
            </div>
          )}
        </div>
      </Modal>
  ...
  const renderTransferOwnerModal = () => {
    return (
      <Modal
        title={t.transfer_owner_title || "Chuyển quyền trưởng nhóm"}
      ...
      <div
        ...
        <div className="flex items-center p-4 border-b border-gray-200 relative">
          ...
          <div className="flex items-center justify-center">
            <h2 className="text-lg font-semibold text-center">
              {t.group_management || "Quản lý nhóm"}
            </h2>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          <div className="mb-4">
            <div className="font-medium mb-3">
              {t.allow_group_members || "Cho phép các thành viên trong nhóm:"}
            </div>
            ...
            <div className="space-y-4">
              ...
              {/* Create notes */}
              <div className="flex items-center justify-between text-gray-500">
                <span>{t.create_notes || "Tạo mới ghi chú, nhắc hẹn"}</span>
                <div className="border rounded-sm w-5 h-5 flex items-center justify-center border-gray-300">
              ...
              {/* Create polls */}
              <div className="flex items-center justify-between text-gray-500">
                <span>{t.create_polls || "Tạo mới bình chọn"}</span>
                <div className="border rounded-sm w-5 h-5 flex items-center justify-center border-gray-300">
              ...
              {/* Send messages */}
              <div className="flex items-center justify-between text-gray-500">
                <span>{t.send_messages || "Gửi tin nhắn"}</span>
                <div className="border rounded-sm w-5 h-5 flex items-center justify-center border-gray-300">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between py-2 text-gray-500">
              <div className="flex items-center">
                <span>
                  {t.approval_mode || "Chế độ phê duyệt thành viên mới"}
                </span>
            ...
            <div className="flex items-center justify-between py-2 border-t border-gray-100 text-gray-500">
              <div className="flex items-center">
                <span>
                  {t.highlight_admin || "Đánh dấu tin nhắn từ trưởng/phó nhóm"}
                </span>
                <QuestionCircleOutlined className="ml-1 text-gray-400" />
              </div>
              <Switch
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        <div className="mb-4">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <span>
                {t.change_name_avatar || "Thay đổi tên & ảnh đại diện của nhóm"}
              </span>
              <div
          ...
          <div className="flex items-center justify-between py-2 border-t border-gray-100">
            <div className="flex items-center">
              <span className="font-medium">
                {t.allow_invitation_link || "Cho phép dùng link tham gia nhóm"}
              </span>
        ...
        <div className="space-y-3">
          <div
            ...
            <span>{t.block_from_group || "Chặn khỏi nhóm"}</span>
          </div>
          ...
          {userRole === UserRole.OWNER && (
            <div
              ...
              <span>{t.owner_deputy || "Trưởng & phó nhóm"}</span>
            </div>
          )}
        </div>
