...
import { SendOutlined, RobotOutlined, UserOutlined, HistoryOutlined, ArrowDownOutlined } from '@ant-design/icons';
...
const AIAssistant: React.FC<AIAssistantProps> = ({ onClose, onSendToChat, inChatMode = false }) => {
  ...
  const loadConversations = async () => {
    try {
      ...
      message.error('Không thể tải lịch sử trò chuyện AI');
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      message.error('Không thể xử lý yêu cầu AI');
    } finally {
      setIsTyping(false);
    }
  };
        ...
        {loadingConversations ? (
          ...
          <List
            ...
            renderItem={(conversation) => {
              ...
              let previewText = "Cuộc trò chuyện mới";
                ...
                >
                  <div className="ai-conversation-info">
                    ...
                    <div className="ai-conversation-preview">
                      ...
                      <div className="ai-conversation-date">{formattedDate}</div>
                    </div>
                  </div>
                </List.Item>
              );
        ...
        <div className="ai-header">
          ...
          <div>
            <Button 
              ...
              className={showHistory ? "ai-history-button-active" : "ai-history-button"}
            >
            </Button>
        ...
        <div className="ai-chat-content">
          ...
          <div className={`ai-chat-main ${showHistory ? 'with-history' : ''}`}>
            <div className="ai-messages-container">
              {!currentConversation?.messages?.length && (
                <div className="ai-welcome-message">
                  ...
                  <p>Bạn có thể hỏi tôi bất cứ điều gì. Tôi sẽ cố gắng giúp đỡ!</p>
                </div>
              )}
              
              {currentConversation?.messages.map((msg, index) => (
                <div key={index} className={`ai-message ${msg.role === 'user' ? 'ai-user-message' : 'ai-assistant-message'}`}>
            ...
            <form onSubmit={handleSubmit} className="ai-input-form">
              ...
              <Button
                type="primary"
                icon={<SendOutlined className={input.trim() ? "ai-send-enabled" : "ai-send-disabled"} />}
    ...
    <div className="ai-assistant-container">
      <div className="ai-sidebar">
        <div className="ai-sidebar-header">
          <h2>AI Assistant</h2>
          <Button type="primary" onClick={startNewConversation}>Tạo mới</Button>
        </div>
        ...
        <div className="ai-conversations-list">
          {loadingConversations ? (
            ...
            <List
              ...
              renderItem={(conversation) => {
                ...
                let preview = "Cuộc trò chuyện mới";
                if (conversation.messages.length > 0) {
                  const firstUserMessage = conversation.messages.find(m => m.role === 'user');
                  preview = firstUserMessage ? firstUserMessage.content : conversation.messages[0].content;
          ...
          {conversations.length === 0 && !loadingConversations && (
            <div className="ai-empty-state">
              ...
              <p>Chưa có cuộc trò chuyện nào</p>
              <p>Hãy bắt đầu bằng cách hỏi AI trợ giúp!</p>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="ai-conversation">
          <div className="ai-messages">
            ...
            {currentConversation?.messages.map((msg, index) => (
              <div key={index} className={`ai-message ${msg.role === 'user' ? 'ai-user-message' : 'ai-assistant-message'}`}>
                <Avatar 
                  icon={msg.role === 'user' ? <UserOutlined /> : <img src="/images/1.png" alt="logo" className='w-10 h-10'/>}
